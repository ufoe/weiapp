...
class WeixinAction extends Action
{
    ...
    private function changeju($data)
    {
        ...
        if ('image' == $data['MsgType']) {
            $pic_url = $data['PicUrl'];
            $url = "http://222.131.155.37:8888/admin-web/api/upload?from=" . $from . "&create_t=" 
            ...
            if ($content == '区域') {
                $url = "http://222.131.155.37:8888/admin-web/api/hotarea";
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                $data = curl_exec($ch);
                ...
                if (!empty($data)){
                    ...
                    $return = array();
                    foreach ($rts as $k => $v) {
                        $return[] = array($v, $k, "http://www.imethodz.com:8888/house/big/0a8ec00a-75bc-4f26-a818-be7cc302ceaf.jpg", "http://222.131.155.37:8888/admin-web/api/area/" .$k ."/1");
                    }
                }
                ...
                $return = array();
                $return[] = array("轻松发布房源", "只需几步填写表单，然后微信发送房源图片即可轻松发布房源", "", "http://222.131.155.37:8888/admin-web/api/pubpage?wechat_id=" . $from);
                return array($return, 'news');
            }
            else if (strpos($content, "找房") === 0) {
                $content = str_replace("找房", "", $content);
                $content = urlencode($content);
                $url = "http://222.131.155.37:8888/admin-web/api/search?key=" . $content;
                Log::write($url);
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                ...
                if (!empty($data)){
                    ...
                    foreach ($rts as $k => $v) {
                        ...
                        $return[] = array($k, $k, "http://www.imethodz.com:8888/house/big/" . $pic, "http://alpha.changeju.com/changzu/" . $longRentId);
                    }
                }
                return array($return, 'news');
            }
                ...
                $content = urlencode($content);
                $url = "http://222.131.155.37:8888/admin-web/api/pub?from=" . $from . "&create_t=" 
